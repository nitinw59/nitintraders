		...
		$(document).ready(function() {
			...
			var buyernameArray = <?php echo json_encode($customercompanynames); ?>;
				...
				$("#showBills").click(function(){
					...
					$.ajax({
                        type:"post",
                        url:"listPaymentsAction.php",
                        data:"company_name="+company_name+"&from_date="+from_date+"&to_date="+to_date+"&action=listMerchantPayments",
						...
						$.each(payments_list, function( index, payment ) {
							...
							var markup= "<tr><td><center>"+payment["MERCHANT_BILL_NUMBER"]+"</center></td><td><center>"+payment["DATE"]+"</center></td><td><center>"+payment["AMOUNT"]+"</center></td><td><center>"+payment["DESCRIPTION"]+"</center></td>";
							
							
							
							total_payment+=parseInt(payment["AMOUNT"]);
					
							
							$("#payments_tbl").append(markup);
						});
						var markup= "<tr><td colspan='2'><center>Total</center></td><td><center>"+total_payment+"</center></td></tr>"
						$("#payments_tbl").append(markup);
						 markup= "<tr><td></td><td></td><td></td><td ><center><button class='print'>Print</button></center></td></tr>"
						$("#payments_tbl").append(markup);
				...
				window.open("printListPayments.php?"+"company_name="+company_name+"&from_date="+from_date+"&to_date="+to_date);
...
include($_SERVER['DOCUMENT_ROOT']."$nitinTraders/index.php");
	...
	$current_due_date=date('Y-m-31', time());
	?>
    ...
    <div class="listPayments" id="listPayments" style="display:none;">
	
	<table id="payments_tbl">
	<tr><th>BILL No </th><th width="15%">DATE </th><th>TOTAL Payment </th><th>dESCRIPTION</th></tr>
