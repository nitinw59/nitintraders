<?php
	...
	$to_date=$_GET["to_date"];
		...
		$TOTAL_PAYMENT=0;
		$TOTAL_PENDING=0;
		$sqlquery="select B.BILL_ID, B.MERCHANT_BILL_NUMBER,B.DATE,B.AMOUNT AS AMOUNT,B.TAX_AMOUNT AS TAX,(B.AMOUNT+B.TAX_AMOUNT) AS TOTAL_AMOUNT , (SELECT SUM(AMOUNT) FROM merchant_payments_tbl WHERE BILL_ID=B.BILL_ID) AS TOTAL_PAYMENT,to_maker,meters,rate,description FROM MERCHANT_BILLS_TBL B,  FABRIC_MERCHANTS_TBL FM WHERE B.FABRIC_MERCHANTS_ID=FM.FABRIC_MERCHANTS_ID AND FM.COMPANY_NAME='".$company_name."' AND B.DATE>='".$from_date."' AND B.DATE<='".$to_date."'";
		$show=mysqli_query($dbhandle,$sqlquery);
