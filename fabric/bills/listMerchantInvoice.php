	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var buyernameArray = <?php echo json_encode($customercompanynames); ?>;
				...
				$("#showBills").click(function(){
					...
					$.ajax({
						...
						$.each(bills_list, function( index, bill ) {
							...
							var markup= "<tr><td><center>"+bill["MERCHANT_BILL_NUMBER"]+"</center></td><td><center>"+bill["DATE"]+"</center></td><td><center>"+bill["to_maker"]+"</center></td><td><center>"+bill["meters"]+"</center></td><td><center>"+bill["rate"]+"</center></td><td><center>"+bill["AMOUNT"]+"</center></td><td><center>"+bill["TAX"]+"</center></td><td><center>"+bill["TOTAL_AMOUNT"]+"</center></td><td><center>"+bill["TOTAL_PAYMENT"]+"</center></td><td><center>"+(bill["TOTAL_AMOUNT"]-bill["TOTAL_PAYMENT"])+"</center></td><td><center><button class='viewbill' value="+bill["BILL_ID"]+" >Bill</button></center></td><td><center><button class='viewpayment' value="+bill["BILL_ID"]+">Payments</button></center></td><td><center><button class='makepayment' value="+bill["BILL_ID"]+">make Payments</button></center></td>";
							
							
							total_amount+=parseInt(bill["TOTAL_AMOUNT"]);
							total_payment+=parseInt(bill["TOTAL_PAYMENT"]);
					
							
							$("#bills_tbl").append(markup);
						});
						var markup= "<tr><td colspan='7'><center>Total</center></td><td><center>"+total_amount+"</center></td><td><center>"+total_payment+"</center></td><td><center>"+(total_amount-total_payment)+"</center></td></tr>"
						$("#bills_tbl").append(markup);
				...
				window.open("../payments/newPayment.php?BILL_ID="+$(this).val(),"_blank");
...
include($_SERVER['DOCUMENT_ROOT']."$server_root/index.php");
	...
	<table id="bills_tbl">
	<tr><th>BILL No </th><th width="15%">DATE </th><th>Maker</th><th>Meter</th><th>Rate</th><th>AMOUNT </th><th>TAX </th><th>TOTAL AMOUNT </th><th>TOTAL Payment </th><th>PENDING </th><th>View Payments </th><th>Make Payment</th></tr>
