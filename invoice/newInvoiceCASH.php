  ...
  <head>
	...
	var duedate=yyyy+"-"+mm+"-"+dd+"";
			...
			$(document).ready(function() {
				...
				//$("#buyername").val("CASH SALE").trigger("change");
				$("#transportname").val("SELF");
				$("#transportparcels").val(1);
				...
				$('#brate').on("keypress", function(e) {
					...
					if (e.keyCode == 13) {
						var factor=1;
						var brate= Math.round($("#brate").val()/1.05);
				...
				$('#rate').on("keypress", function(e) {
					...
					if (e.keyCode == 13) {
						...
						var rate=$("#rate").val();
						var b_rate=current_item_rate-(rate* 1.05);
						if(b_rate<0)
							b_rate=0;
						$("#brate").val(b_rate);
						$("#brate").focus();
